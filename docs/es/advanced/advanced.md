...
- [Implementación como servicios públicos](#implementación-como-servicios-públicos)
...
| `--<Servicios>`                  | Usar [**servicio específico**](./Documentación-de-servicios-de-traducción.md) para la traducción | `pdf2zh example.pdf --openai`<br>`pdf2zh example.pdf --deepseek`                                                     |
...
| `--rpc-doclayout`               | Dirección del host del servicio RPC para el análisis del diseño del documento                                  |                                                                                                                      |
...
| `--custom-system-prompt`        | Indicación personalizada del sistema para traducción. Se utiliza para `/no_think` en Qwen 3                   | `pdf2zh example.pdf --custom-system-prompt "/no_think You are a professional, authentic machine translation engine"` |
...
| `--primary-font-family`         | Anular la familia de fuentes principal para el texto traducido. Opciones: 'serif' para fuentes serif, 'sans-serif' para fuentes sans-serif, 'script' para fuentes script/cursivas. Si no se especifica, utiliza la selección automática de fuentes basada en las propiedades del texto original. | `pdf2zh example.pdf --primary-font-family serif` |
...
| `--ocr-workaround`              | Forzar que el texto traducido sea negro y agregar un fondo blanco                             | `pdf2zh example.pdf --ocr-workaround`                                                                                |
| `--auto-enable-ocr-workaround`  | Habilitar solución automática para OCR. Si se detecta que un documento está muy escaneado, intentará habilitar el procesamiento OCR y omitirá más detección de escaneo. Consulte la documentación para más detalles. (predeterminado: False) | `pdf2zh example.pdf --auto-enable-ocr-workaround True`                    |
...
Consulte [Códigos de idioma de Google](https://developers.google.com/admin-sdk/directory/v1/languages), [Códigos de idioma de DeepL](https://developers.deepl.com/docs/resources/supported-languages)
...
Conservar las fuentes `Latex`, `Mono`, `Code`, `Italic`, `Symbol` y `Math` por defecto:
...
>
> Las modificaciones de mayor rango anularán las de menor rango.
>
> **cli/gui > env > archivo de configuración del usuario > archivo de configuración predeterminado**

- Modificar la configuración mediante **Argumentos de la línea de comandos**
...
Por ejemplo, si deseas habilitar una ventana de GUI, puedes usar el siguiente comando:

```bash
pdf2zh_next --gui
```

- Modificar la configuración mediante **variables de entorno**
...
PDFMathTranslate almacena los textos traducidos en caché para aumentar la velocidad y evitar llamadas API innecesarias para los mismos contenidos. Puedes usar la opción `--ignore-cache` para ignorar la caché de traducción y forzar la retraducción.
...
#### Implementación como servicios públicos

Al implementar una GUI de pdf2zh en servicios públicos, debes modificar el archivo de configuración como se describe a continuación.
...
Al usar Autenticación y página de bienvenida para especificar qué usuario utilizará la interfaz web y personalizar la página de inicio de sesión:
