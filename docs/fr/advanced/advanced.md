...
| `--restore-offline-assets`      | Restaurer le package d'actifs hors ligne à partir du répertoire spécifié               | `pdf2zh example.pdf --restore-offline-assets /path`                                                                  |
...
| `--rpc-doclayout`               | Adresse du service RPC pour l'analyse de la mise en page des documents                                  |                                                                                                                      |
...
| `--auto-enable-ocr-workaround`  | Active le contournement automatique de l'OCR. Si un document est détecté comme étant fortement scanné, cette option tentera d'activer le traitement OCR et ignorera toute détection de scan supplémentaire. Voir la documentation pour plus de détails. (par défaut : False) | `pdf2zh example.pdf --auto-enable-ocr-workaround True`                    |
...
Cette commande traduira la première page, la troisième page, les pages 10 à 20, et toutes les pages à partir de la 25 jusqu'à la fin.
...
Voir [Codes de langue de Google](https://developers.google.com/admin-sdk/directory/v1/languages), [Codes de langue de DeepL](https://developers.deepl.com/docs/resources/supported-languages)
...
Invite personnalisée du système pour la traduction. Elle est principalement utilisée pour ajouter l'instruction '/no_think' de Qwen 3 dans l'invite.
...
- Modification de la configuration via **Arguments de la ligne de commande**

Dans la plupart des cas, vous pouvez directement transmettre vos paramètres souhaités via les arguments de la ligne de commande. Veuillez vous référer à [Arguments de la ligne de commande](#cmd) pour plus d'informations.
...
Vous pouvez remplacer les `--` dans les arguments de la ligne de commande par `PDF2ZH_`, connecter les paramètres en utilisant `=`, et remplacer `-` par `_` comme variables d'environnement.
...
Il est fortement recommandé de se référer au contenu de ce fichier de configuration et d'utiliser le **Fichier de configuration personnalisé** pour implémenter votre propre fichier de configuration.
...
Lors de l'utilisation de l'Authentification et page d'accueil pour spécifier quel utilisateur doit utiliser l'interface Web et personnaliser la page de connexion :
...
PDFMathTranslate prend en charge le tableau de glossaire. Le fichier des tableaux de glossaire doit être un fichier `csv`.
Il y a trois colonnes dans le fichier. Voici un exemple de fichier de glossaire :

| source | target  | tgt_lng |
|--------|---------|---------|
| AutoML | ML automatique  | fr-FR   |
| a,a    | a       | fr-FR   |
| "      | "       | fr-FR   |
