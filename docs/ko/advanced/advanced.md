...
| `--pool-max-worker`             | 번역 풀의 최대 작업자 수. 설정하지 않으면 qps 를 작업자 수로 사용합니다 | `pdf2zh example.pdf --pool-max-worker 100`                                                                |
| `--no-auto-extract-glossary`    | 용어집 자동 추출 비활성화                                                          | `pdf2zh example.pdf --no-auto-extract-glossary`                                                                      |
| `--primary-font-family`         | 번역된 텍스트의 기본 글꼴 패밀리를 재정의합니다. 선택 사항: 'serif'는 세리프 글꼴, 'sans-serif'는 산세리프 글꼴, 'script'는 스크립트/이탤릭 글꼴입니다. 지정되지 않은 경우 원본 텍스트 속성을 기반으로 자동 글꼴 선택이 사용됩니다. | `pdf2zh example.pdf --primary-font-family serif` |
...
| `--ocr-workaround`              | 번역된 텍스트를 강제로 검은색으로 만들고 흰색 배경 추가                             | `pdf2zh example.pdf --ocr-workaround`                                                                                |
...
이 매개변수가 True 로 설정되면 PDF 클리닝 단계가 건너뛰어져 호환성이 향상되고 일부 글꼴 처리 문제를 피할 수 있습니다.
...
> - 사용 가능한 서비스는 *영문 쉼표* <kbd>,</kbd>로 구분하세요.
...
인증 및 환영 페이지를 사용하여 Web UI 를 사용할 사용자와 로그인 페이지를 사용자 지정할 때:
