...
명령줄에서 번역 명령을 실행하여 현재 작업 디렉토리에 번역된 문서 `example-mono.pdf`와 이중 언어 문서 `example-dual.pdf`를 생성합니다. 기본 번역 서비스로 Google을 사용합니다. 더 많은 지원되는 번역 서비스는 [여기](https://github.com/PDFMathTranslate/PDFMathTranslate-next/blob/main/docs/ADVANCED.md#services)에서 찾을 수 있습니다.
...
| `--output`                      | 출력 디렉토리 지정                                                             | `pdf2zh example.pdf --output /outputpath`                                                                            |
...
| `--generate-offline-assets`     | 지정된 디렉토리에 오프라인 자산 패키지 생성                             | `pdf2zh example.pdf --generate-offline-assets /path`                                                                 |
...
| `--pages`                       | 문서 부분 번역                                                           | `pdf2zh example.pdf --pages 1,2,1-,-3,3-5`                                                                           |
...
| `--primary-font-family`         | 번역된 텍스트의 기본 글꼴 패밀리를 재정의합니다. 선택 사항: 'serif'는 세리프 글꼴, 'sans-serif'는 산세리프 글꼴, 'script'는 스크립트/이탤릭 글꼴입니다. 지정되지 않은 경우 원본 텍스트 속성을 기반으로 자동 글꼴 선택을 사용합니다. | `pdf2zh example.pdf --primary-font-family serif` |
...
| `--max-pages-per-part`          | 부분 번역당 최대 페이지 수                                           | `pdf2zh example.pdf --max-pages-per-part 1`                                                                          |
...
| `--ocr-workaround`              | 번역된 텍스트를 강제로 검은색으로 만들고 흰색 배경을 추가합니다                             | `pdf2zh example.pdf --ocr-workaround`                                                                                |
...
구성 파일을 수정하고 가져오는 여러 가지 방법이 있습니다.
...
이 매개변수가 True로 설정되면, PDF 클리닝 단계가 건너뛰어져 호환성을 향상시키고 일부 폰트 처리 문제를 피할 수 있습니다.
...
PDFMathTranslate는 동일한 콘텐츠에 대한 불필요한 API 호출을 줄이고 속도를 높이기 위해 번역된 텍스트를 캐시합니다. `--ignore-cache` 옵션을 사용하여 번역 캐시를 무시하고 강제로 재번역할 수 있습니다.
...
> - 사용 가능한 서비스를 구분할 때는 *영문 쉼표* <kbd>,</kbd> 를 사용하세요.
...
예시 auth.txt
각 줄에는 사용자 이름과 비밀번호가 쉼표로 구분되어 있습니다.
...
PDFMathTranslate는 용어 테이블을 지원합니다. 용어 테이블 파일은 `csv` 파일이어야 합니다.  
파일에는 세 개의 열이 있습니다. 다음은 데모 용어 파일입니다:
...
CLI 사용자의 경우:
용어집에 여러 파일을 사용할 수 있습니다. 그리고 다른 파일은 `,`로 구분해야 합니다.
