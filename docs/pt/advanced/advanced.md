...
| `--restore-offline-assets`      | Restaurar pacote de ativos offline do diretório especificado                            | `pdf2zh example.pdf --restore-offline-assets /caminho`                                                                  |
...
Na maioria dos casos, você pode passar diretamente suas configurações desejadas por meio de argumentos da linha de comando. Consulte [Argumentos da Linha de Comando](#argumentos-da-linha-de-comando) para obter mais informações.
...
PDFMathTranslate armazena em cache os textos traduzidos para aumentar a velocidade e evitar chamadas desnecessárias à API para conteúdos iguais. Você pode usar a opção `--ignore-cache` para ignorar o cache de tradução e forçar a retradução.
