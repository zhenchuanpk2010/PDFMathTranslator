...
| `--restore-offline-assets`      | Restaurar o pacote de ativos offline do diretório especificado                            | `pdf2zh example.pdf --restore-offline-assets /path`                                                                  |
...
| `--primary-font-family`         | Substitui a família de fontes primária para o texto traduzido. Opções: 'serif' para fontes serifadas, 'sans-serif' para fontes sem serifa, 'script' para fontes script/itálicas. Se não especificado, usa a seleção automática de fontes com base nas propriedades do texto original. | `pdf2zh example.pdf --primary-font-family serif` |
...
| `--disable-config-auto-save`    | Desativar o salvamento automático da configuração | `pdf2zh --gui --disable-config-auto-save`       |
| `--server-port`                 | Porta da WebUI                             | `pdf2zh --gui --server-port 7860`               |
...
Use expressões regulares para especificar fontes de fórmulas e caracteres que precisam ser preservados:

```bash
pdf2zh_next example.pdf --formular-font-pattern "(CM[^RT].*|MS.*|.*Ital)" --formular-char-pattern "(\(|\||\)|\+|=|\d|[\u0080-\ufaff])"
```

Preservar fontes `Latex`, `Mono`, `Code`, `Italic`, `Symbol` e `Math` por padrão:
...
- Modificando a configuração por meio de **Variáveis de Ambiente**
...
> [!TIP]
> - Por padrão, o pdf2zh 2.0 salva automaticamente a configuração atual em `~/.config/pdf2zh/config.v3.toml` cada vez que você clica no botão de tradução na GUI. Esse arquivo de configuração será carregado por padrão na próxima inicialização.
...
> - Os arquivos de configuração podem incluir números de versão como "v2", "v3", etc. Esses são **números de versão do arquivo de configuração**, **não** o número de versão do pdf2zh em si.
...
Agora você pode enviar seu próprio arquivo de glossário. Após enviar o arquivo, você pode verificar o conteúdo clicando no nome do arquivo e o conteúdo será exibido abaixo.
