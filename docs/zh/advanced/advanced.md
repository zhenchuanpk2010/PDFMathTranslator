...
在命令行中执行翻译命令，在当前工作目录下生成翻译后的文档 `example-mono.pdf` 和双语文档 `example-dual.pdf`。默认使用谷歌作为翻译服务。更多支持的翻译服务可查看[此处](https://github.com/PDFMathTranslate/PDFMathTranslate-next/blob/main/docs/ADVANCED.md#services)。
...
| `--<Services>`                  | 使用[**特定服务**](./翻译服务文档)进行翻译 | `pdf2zh example.pdf --openai`<br>`pdf2zh example.pdf --deepseek`                                                     |
| `--help`, `-h`                  | 显示帮助信息并退出                                                             | `pdf2zh -h`                                                                                                          |
| `--config-file`                 | 配置文件路径                                                         | `pdf2zh --config-file /path/to/config/config.toml`                                                                   |
| `--report-interval`             | 进度报告间隔时间（秒）                                                    | `pdf2zh example.pdf --report-interval 5`                                                                             |
| `--debug`                       | 使用调试日志级别                                                                | `pdf2zh example.pdf --debug`                                                                                         |
| `--gui`                         | 与图形界面交互                                                                      | `pdf2zh --gui`                                                                                                       |
| `--warmup`                      | 仅下载并验证所需资源后退出                                     | `pdf2zh example.pdf --warmup`                                                                                        |
| `--generate-offline-assets`     | 在指定目录下生成离线资源包                             | `pdf2zh example.pdf --generate-offline-assets /path`                                                                 |
...
| `--custom-system-prompt`        | 翻译的自定义系统提示。用于 Qwen 3 中的 `/no_think`                   | `pdf2zh example.pdf --custom-system-prompt "/no_think You are a professional, authentic machine translation engine"` |
...
| `--primary-font-family`         | 覆盖翻译文本的主字体族。可选值：'serif' 表示衬线字体，'sans-serif' 表示无衬线字体，'script' 表示手写体/斜体字体。若未指定，则根据原文属性自动选择字体。 | `pdf2zh example.pdf --primary-font-family serif` |
...
| `--max-pages-per-part`          | 每次拆分翻译的最大页数                                           | `pdf2zh example.pdf --max-pages-per-part 1`                                                                          |
...
翻译时的自定义系统提示。主要用于在提示中添加 Qwen 3 的 `/no_think` 指令。
...
在大多数情况下，您可以直接通过命令行参数传递所需的设置。更多信息请参阅[命令行参数](#cmd)。
...
强烈建议参考此配置文件内容，并使用**自定义配置**来实现自己的配置文件。
...
在将 pdf2zh 图形界面部署为公共服务时，您应按照以下说明修改配置文件。
...
现在您可以上传自己的术语表文件。上传文件后，点击文件名即可查看其内容，内容将显示在下方。
