...
| `--output`                      | 输出文件的目录                                                             | `pdf2zh example.pdf --output /outputpath`                                                                            |
...
| `--custom-system-prompt`        | 用于翻译的自定义系统提示。在 Qwen 3 中用于 `/no_think`                   | `pdf2zh example.pdf --custom-system-prompt "/no_think You are a professional, authentic machine translation engine"` |
...
| `--primary-font-family`         | 覆盖翻译文本的主要字体家族。可选值：'serif' 表示衬线字体，'sans-serif' 表示无衬线字体，'script' 表示手写体/斜体字体。如果未指定，则根据原文属性自动选择字体。 | `pdf2zh example.pdf --primary-font-family serif` |
