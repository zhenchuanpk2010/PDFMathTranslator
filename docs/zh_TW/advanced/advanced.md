...
在命令行中執行翻譯指令，於當前工作目錄下生成翻譯後的文檔 `example-mono.pdf` 與雙語文檔 `example-dual.pdf`。預設使用 Google 作為翻譯服務。更多支援的翻譯服務可參考 [HERE](https://github.com/PDFMathTranslate/PDFMathTranslate-next/blob/main/docs/ADVANCED.md#services)。
...
| `--output`                      | 輸出檔案的目錄                                                             | `pdf2zh example.pdf --output /outputpath`                                                                            |
...
| `--formular-char-pattern`       | 用於識別公式文字的字符模式                                             | `pdf2zh example.pdf --formular-char-pattern "(MS.*)"`                                                                |
...
| `--max-pages-per-part`          | 每次分割翻譯的最大頁數                                           | `pdf2zh example.pdf --max-pages-per-part 1`                                                                          |
...
| `--glossaries`                  | 用於翻譯的自定義詞彙表。                                                      | `pdf2zh example.pdf --glossaries "glossary1.csv,glossary2.csv,glossary3.csv"`                                         |
...
預設保留 `Latex`、`Mono`、`Code`、`Italic`、`Symbol` 和 `Math` 字體：
...
翻譯用的自訂系統提示。主要用於在提示中加入 Qwen 3 的 `/no_think` 指令。
...
> - `default` 目錄中的配置文件由程序自動生成。您可以複製它們進行修改，但請勿直接修改。
...
當使用 認證與歡迎頁面 來指定哪些使用者可以使用 Web UI 並自訂登入頁面時：
