[**高級選項**](./introduction.md) > **高級選項** _(當前)_
...
| `--primary-font-family`         | 覆蓋翻譯文字的主要字體家族。選項：'serif' 用於襯線字體，'sans-serif' 用於無襯線字體，'script' 用於手寫/斜體字體。若未指定，則根據原始文字屬性自動選擇字體。 | `pdf2zh example.pdf --primary-font-family serif` |
| `--no-dual`                     | 不輸出雙語 PDF 檔案                                                      | `pdf2zh example.pdf --no-dual`                                                                                       |
| `--no-mono`                     | 不輸出單語種 PDF 檔案                                                    | `pdf2zh example.pdf --no-mono`                                                                                       |
...
| `--auto-enable-ocr-workaround`  | 啟用自動 OCR 解決方案。若偵測到文件為重度掃描檔，將嘗試啟用 OCR 處理並跳過後續掃描偵測。詳見文檔說明。(預設值：False) | `pdf2zh example.pdf --auto-enable-ocr-workaround True`                    |
...
| `--glossaries`                  | 用於翻譯的自訂詞彙表。                                                      | `pdf2zh example.pdf --glossaries "glossary1.csv,glossary2.csv,glossary3.csv"`                                         |
...
| `--disable-gui-sensitive-input` | 停用圖形界面敏感輸入            | `pdf2zh --gui --disable-gui-sensitive-input`    |
...
自訂系統提示用於翻譯。主要用於在提示中加入 Qwen 3 的 `/no_think` 指令。
...
> **配置文件層級**
>
> 當使用不同方法修改同一參數時，軟體將依照以下優先級順序套用變更。
>
> 較高優先級的修改會覆蓋較低優先級的設定。
>
> **cli/gui > env > 使用者設定檔 > 預設設定檔**

- 透過 **命令行參數** 修改配置

在大多數情況下，您可以直接通過命令行參數傳遞所需的設置。更多資訊請參閱[命令行參數](#命令行參數)。
...
強烈建議參考此設定檔的內容，並使用**自訂配置**來實作您自己的設定檔。

> [!TIP]
> - 預設情況下，pdf2zh 2.0 每次在圖形界面點擊翻譯按鈕時，會自動將當前配置保存至 `~/.config/pdf2zh/config.v3.toml`。此配置文件將在下次啟動時自動載入。
...
當此參數設為 True 時，將跳過 PDF 清理步驟，這可提升相容性並避免部分字體處理問題。
...
PDFMathTranslate 會快取已翻譯的文字，以提升速度並避免對相同內容進行不必要的 API 呼叫。你可以使用 `--ignore-cache` 選項來忽略翻譯快取並強制重新翻譯。
...
在公共服務上部署 pdf2zh 圖形界面時，您應按照以下說明修改配置檔案。
