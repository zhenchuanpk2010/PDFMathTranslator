...
| `--short-line-split-factor`     | Фактор порога разделения для коротких строк                                                 |                                                                                                                      |
...
| `--watermark-output-mode`       | Режим вывода водяного знака для PDF-файлов                                                    | `pdf2zh example.pdf --watermark-output-mode "NoWaterMark"`                                                           |
...
| `--welcome-page`                | Путь к файлу приветственной страницы HTML          | `pdf2zh --gui --welcome-page /path`             |
...
>
> При изменении одного и того же параметра разными способами, программное обеспечение будет применять изменения в соответствии с порядком приоритета, указанным ниже.
>
...
При развертывании графического интерфейса pdf2zh в публичных сервисах следует изменить конфигурационный файл, как описано ниже.
...
При использовании Аутентификация и приветственная страница для указания, какой пользователь может использовать Web UI и настройки страницы входа:
