...
- [Развертывание в качестве публичных сервисов](#развертывание-в-качестве-публичных-сервисов)
...
Выполните команду перевода в командной строке, чтобы сгенерировать переведенный документ `example-mono.pdf` и двуязычный документ `example-dual.pdf` в текущей рабочей директории. Используйте Google в качестве сервиса перевода по умолчанию. Дополнительные поддерживаемые сервисы перевода можно найти [ЗДЕСЬ](https://github.com/PDFMathTranslate/PDFMathTranslate-next/blob/main/docs/ADVANCED.md#services).
...
| `--debug`                       | Использовать уровень логирования отладки                                                | `pdf2zh example.pdf --debug`                                                                                         |
...
>
> **cli/gui > env > пользовательский конфигурационный файл > конфигурационный файл по умолчанию**
...
> - Файлы конфигурации могут включать версионные номера, такие как "v2", "v3" и т.д. Это **версионные номера файлов конфигурации**, **а не** версионный номер самого pdf2zh.
...
PDFMathTranslate кэширует переведенные тексты для увеличения скорости и избежания ненужных вызовов API для одинаковых содержимых. Вы можете использовать опцию `--ignore-cache` для игнорирования кэша перевода и принудительного повторного перевода.
...
#### Развертывание в качестве публичных сервисов

При развертывании графического интерфейса pdf2zh на публичных сервисах, вам следует изменить файл конфигурации, как описано ниже.
...
Теперь вы можете загружать собственный файл глоссария. После загрузки файла вы можете проверить его содержимое, нажав на его имя, и содержимое отобразится ниже.
