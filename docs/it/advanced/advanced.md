[**Opzioni avanzate**](./introduction.md) > **Opzioni avanzate** _(attuale)_
...
| `--custom-system-prompt`        | Prompt personalizzato del sistema per la traduzione. Utilizzato per `/no_think` in Qwen 3                   | `pdf2zh example.pdf --custom-system-prompt "/no_think You are a professional, authentic machine translation engine"` |
...
| `--formular-char-pattern`       | Modello di caratteri per identificare il testo delle formule                                             | `pdf2zh example.pdf --formular-char-pattern "(MS.*)"`                                                                |
...
Questo comando tradurrà la prima pagina, la terza pagina, le pagine da 10 a 20 e tutte le pagine da 25 fino alla fine.
...
> [!NOTE]
> **Gerarchia del file di configurazione**
>
...
- Modificare la configurazione tramite **Argomenti della riga di comando**

Nella maggior parte dei casi, puoi passare direttamente le impostazioni desiderate attraverso gli argomenti della riga di comando. Consulta [Argomenti della riga di comando](#cmd) per ulteriori informazioni.
...
- Modificare la configurazione tramite **Variabili d'ambiente**
...
Quando si utilizza Autenticazione e pagina di benvenuto per specificare quale utente utilizzare l'interfaccia Web UI e personalizzare la pagina di accesso:
...
È possibile utilizzare più file per il glossario. I diversi file devono essere separati da `,`.
